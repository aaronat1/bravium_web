...
import { generateBraviumPdf } from "@/lib/pdf-service";
...
const formatRecipients = (data: Record<string, any>) => {
...
function ViewCredentialDialog({ credential, isOpen, onOpenChange }: { credential: IssuedCredential | null, isOpen: boolean, onOpenChange: (open: boolean) => void }) {
    ...
    const generateCredentialPdf = async (): Promise<Blob> => {
        ...
        return generateBraviumPdf({
            title: credential.templateName,
            jws: credential.jws,
            qrCodeDataUrl,
            recipientdata: formatRecipients(credential.recipientData),
            issuedAt: credential.issuedAt ? format(credential.issuedAt.toDate(), 'PPP', { locale: enUS }) : undefined, // Localization can be handled better if needed
            id: credential.id
        });
    };
...
export default function CredentialsPage() {
    ...
    const formatRecipientData = (recipientData: Record<string, any>) => {
        return formatRecipients(recipientData);
    };
